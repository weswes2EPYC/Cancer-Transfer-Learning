...
import_model = True
dataset_num = 7500
experiment_load = 5000


def calculate_performance(predictions, labels):
    metrics_output = precision_recall_fscore_support(labels, predictions, labels=(0,1,2), zero_division = 0)
...
data_path = 'lung_colon_image_set\\lung_image_sets'
...
writer = SummaryWriter(f'tensorboard_output/lung_cancer/{Type}_lung_{dataset_num}')
...
if import_model == True:
    ...
    net.load_state_dict(torch.load(f'saved_models/colon_cancer/scratch_colon_{experiment_load}',map_location=torch.device(device)))
...
training_metrics = {
    ...
    'precision_scc': [], 'recall_scc': [], 'f1_scc': []
}
validation_metrics = {
    'loss': [], 'precision_aca': [], 'recall_aca': [], 'f1_aca': [], 'accuracy': [],
    'precision_scc': [], 'recall_scc': [], 'f1_scc': [],
...
f = open(f'{Type}_lung_{dataset_num}.txt', 'w')
...
for epoch in range(15):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        for i,label in enumerate(['aca', 'benign', 'scc']):
        ...
        if itergrad % 10 == 0:
            ...
            if average_loss < lowest_vloss:
                ...
                path = f'saved_models/lung_cancer/{Type}_lung_{dataset_num}'
